...
class NX47_VESU:
    ...
    def run_inference(self):
        ...
        # Path resolution logic for Kaggle environment
        test_path = os.path.join(self.data_path, 'test')
        train_path = os.path.join(self.data_path, 'train')
        
        # Priority: Test fragments, then Train fragments (for submission testing)
        if os.path.exists(test_path):
            ...
            self.log_event("WARNING", "Test path not found, falling back to train path for validation")
            test_fragments = [d for d in os.listdir(train_path) if os.path.isdir(os.path.join(train_path, d))][:1] # Use 1 train fragment for testing
            test_path = train_path
        else:
            self.log_event("ERROR", f"Neither test nor train path found in {self.data_path}")
            test_fragments = []
        ...
        for frag in test_fragments:
            ...
            # Robust layer loading
            layers_glob = os.path.join(test_path, frag, 'layers', '*.tif')
            layers = sorted(glob.glob(layers_glob))
            if not layers:
                self.log_event("WARNING", f"No layers found for fragment {frag}")
                results.append({"Id": frag, "Predicted": ""})
                continue
                
            layers = layers[:3] # POC: 3 layers
            ...
            for layer_path in layers:
                try:
                    ...
                    self.log_event("LAYER_ERROR", f"Error processing {layer_path}: {str(e)}")
