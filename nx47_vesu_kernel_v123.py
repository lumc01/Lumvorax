...
class V123Config:
    ...
    # V123: meta-neuron / evolutionary controls
    meta_neurons: int = 3
...
def train_nx47_autonomous(features: np.ndarray, cfg: V123Config, rng: np.random.Generator, memory: NX47EvolutionMemory | None = None) -> Tuple[NX47AtomNeuron, Dict[str, Any]]:
...
def hysteresis_topology_3d(prob: np.ndarray, cfg: V123Config) -> np.ndarray:
...
class NX47V123Kernel:
    def __init__(self, root: Path = Path('/kaggle/input/competitions/vesuvius-challenge-surface-detection'), output_dir: Path = Path('/kaggle/working'), config: V123Config | None = None) -> None:
        self.version = 'NX47 V123'
        self.root = self._resolve_root(root)
        self.test_dir = self.root / 'test_images'
        self.output_dir = output_dir
        self.tmp_dir = output_dir / 'tmp_masks_v123'
        self.overlay_dir = output_dir / 'overlays_v123'
        self.submission_path = output_dir / 'submission.zip'
        self.roadmap_path = output_dir / 'v123_roadmap_realtime.json'
        self.logs_path = output_dir / 'v123_execution_logs.json'
        self.memory_path = output_dir / 'v123_memory_tracker.json'
        self.metadata_path = output_dir / 'v123_execution_metadata.json'
        self.ultra_log_path = output_dir / 'v123_ultra_authentic_360_merkle.jsonl'

        self.cfg = config or V123Config()
