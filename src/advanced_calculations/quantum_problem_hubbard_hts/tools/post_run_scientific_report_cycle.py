...
def main():
    ...
    traceability_pct = 100.0 if (logs / "checksums.sha256").exists() else 0.0
    ...
    lines.append("- **Contexte** : pipeline multi-modules (Hubbard, QCD proxy, Quantum Field, Dense Nuclear, Quantum Chemistry).")
    lines.append("- **Hypothèses** : la dynamique proxy doit rester stable, reproductible et traçable sous variations de paramètres (step, dt, contrôles externes).")
    ...
    lines.append("- Exécutés: reproductibilité, convergence, stress, contrôle phase/pump/quench, sweep dt, FFT, stabilité Von Neumann proxy, cas jouet analytique, dérivées temporelles/variance.")
