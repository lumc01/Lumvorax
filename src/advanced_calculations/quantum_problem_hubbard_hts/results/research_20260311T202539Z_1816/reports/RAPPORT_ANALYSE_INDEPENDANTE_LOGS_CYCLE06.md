# Rapport d’analyse indépendante des logs (hors rapport pipeline)

Run analysé: `research_20260311T202539Z_1816`
Horodatage UTC: `2026-03-11T20:34:47.183620+00:00`

## Phase 1 — Synchronisation et intégrité
- Revue indépendante effectuée à partir des artefacts bruts du run (`logs/*.csv`, `tests/*.csv`).
- Fichier checksum global présent: **non**.

## Phase 2 — Analyse des données
- Lignes métriques brutes: **305**
- Trace normalisée: **52**
- Questions expertes: **19**
- Modules metadata: **13**
- Moyenne énergie brute: **1.372658**
- Moyenne pairing brut: **0.259621**
- Moyenne sign ratio brut: **0.000117**

## Phase 3 — Vérification exhaustive
- Vérification famille par famille des tests PASS/FAIL/OBSERVED:
  - `benchmark`: PASS=0 FAIL=7 OBSERVED=0 (pass=0.00%)
  - `cluster_scale`: PASS=4 FAIL=0 OBSERVED=32 (pass=11.11%)
  - `control`: PASS=3 FAIL=0 OBSERVED=0 (pass=100.00%)
  - `convergence`: PASS=5 FAIL=0 OBSERVED=0 (pass=100.00%)
  - `dt_sweep`: PASS=1 FAIL=0 OBSERVED=3 (pass=25.00%)
  - `dynamic_pumping`: PASS=0 FAIL=0 OBSERVED=4 (pass=0.00%)
  - `exact_solver`: PASS=1 FAIL=0 OBSERVED=2 (pass=33.33%)
  - `physics`: PASS=2 FAIL=0 OBSERVED=0 (pass=100.00%)
  - `reproducibility`: PASS=1 FAIL=1 OBSERVED=0 (pass=50.00%)
  - `sensitivity`: PASS=0 FAIL=0 OBSERVED=8 (pass=0.00%)
  - `spectral`: PASS=2 FAIL=0 OBSERVED=0 (pass=100.00%)
  - `stability`: PASS=2 FAIL=0 OBSERVED=0 (pass=100.00%)
  - `stress`: PASS=1 FAIL=0 OBSERVED=0 (pass=100.00%)
  - `verification`: PASS=0 FAIL=1 OBSERVED=0 (pass=0.00%)

## Phase 4 — Analyse scientifique
- Couverture questions expertes complètes: **12/19 (63.16%)**
- Stabilité numérique (suite dédiée): **56.67% PASS**
- Points en échec détectés et conservés (pas masqués):
  - `new_tests_results`: reproducibility/rep_diff_seed -> FAIL (value=0.00000000000000)
  - `new_tests_results`: verification/independent_calc -> FAIL (value=6.5445062145)
  - `new_tests_results`: benchmark/qmc_dmrg_rmse -> FAIL (value=1284424.3417498153)
  - `new_tests_results`: benchmark/qmc_dmrg_mae -> FAIL (value=810133.7814389415)
  - `new_tests_results`: benchmark/qmc_dmrg_within_error_bar -> FAIL (value=0.000000)
  - `new_tests_results`: benchmark/qmc_dmrg_ci95_halfwidth -> FAIL (value=650008.4004478998)
  - `new_tests_results`: benchmark/external_modules_rmse -> FAIL (value=30625.5347172832)
  - `new_tests_results`: benchmark/external_modules_mae -> FAIL (value=20344.4631734003)
  - `new_tests_results`: benchmark/external_modules_within_error_bar -> FAIL (value=0.000000)
  - `numerical_stability_suite`: energy_conservation::hubbard_hts_core -> FAIL (energy_density_drift_max=0.0000030227)
  - `numerical_stability_suite`: energy_conservation::qcd_lattice_proxy -> FAIL (energy_density_drift_max=0.0000050817)
  - `numerical_stability_suite`: energy_conservation::quantum_field_noneq -> FAIL (energy_density_drift_max=0.0000039201)
  - `numerical_stability_suite`: energy_conservation::dense_nuclear_proxy -> FAIL (energy_density_drift_max=0.0000064992)
  - `numerical_stability_suite`: energy_conservation::quantum_chemistry_proxy -> FAIL (energy_density_drift_max=0.0000020409)
  - `numerical_stability_suite`: energy_conservation::spin_liquid_exotic -> FAIL (energy_density_drift_max=0.0000039284)
  - `numerical_stability_suite`: energy_conservation::topological_correlated_materials -> FAIL (energy_density_drift_max=0.0000024115)
  - `numerical_stability_suite`: energy_conservation::correlated_fermions_non_hubbard -> FAIL (energy_density_drift_max=0.0000035590)
  - `numerical_stability_suite`: energy_conservation::multi_state_excited_chemistry -> FAIL (energy_density_drift_max=0.0000022232)
  - `numerical_stability_suite`: energy_conservation::bosonic_multimode_systems -> FAIL (energy_density_drift_max=0.0000027196)
  - `numerical_stability_suite`: energy_conservation::multiscale_nonlinear_field_models -> FAIL (energy_density_drift_max=0.0000036636)
  - `numerical_stability_suite`: energy_conservation::far_from_equilibrium_kinetic_lattices -> FAIL (energy_density_drift_max=0.0000033059)
  - `numerical_stability_suite`: energy_conservation::multi_correlated_fermion_boson_networks -> FAIL (energy_density_drift_max=0.0000027705)

## Phase 5 — Interprétation pédagogique
- Un test `PASS` signifie que le critère numérique codé est satisfait.
- Un test `FAIL` signifie qu’une hypothèse n’est pas validée avec les paramètres actuels.
- Un test `OBSERVED` signifie valeur mesurée sans gate binaire stricte.

## Phase 6 — Réponse point par point
- **Question**: Les valeurs et % sont-elles à jour ?
  - **Analyse**: recalcul direct depuis fichiers du run.
  - **Réponse**: Oui, ce rapport recalcule tous les ratios au moment de l’exécution.
  - **Solution**: conserver ce rapport indépendant dans chaque nouveau run.
- **Question**: Les nouvelles questions sont-elles bien incluses ?
  - **Analyse**: lecture de `expert_questions_matrix.csv` et comptage des statuts.
  - **Réponse**: Oui, incluses et quantifiées ici.
  - **Solution**: ajouter un test bloquant si question obligatoire absente.

## Phase 7 — Correctifs proposés
1. Réduire dt et augmenter steps uniquement sur modules/tests explicitement en FAIL.
2. Étendre la campagne multi-seed ciblée sur les familles en échec (pas sur les PASS).
3. Ajouter/resserrer benchmarks externes individuels pour les modules encore partiels.

## Phase 8/9 — Intégration technique et traçabilité
- Rapport indépendant écrit dans `reports/`.
- Résumé machine en JSON + checksums SHA256 dédiés écrits dans `logs/`.

## Phase 10 — Commande reproductible
```bash
bash src/advanced_calculations/quantum_problem_hubbard_hts/run_research_cycle.sh
```

## État d’avancement vers la solution (%)
- Score pondéré indépendant: **63.47%**
