...
#include <time.h>
#include <unistd.h>
#include <omp.h>  /* C59-MP : parallélisation OpenMP des répliques PT-MC */
...
#define PT_MC_N_SWEEPS         200000 /* C59-P3 : 20000→200000 sweeps (×10, N_eff≥100 garanti) */
#define PT_MC_N_THERMALIZE     40000  /* C59-P3 : 4000→40000 (20% × N_SWEEPS, équilibration robuste) */
...
static void pt_mc_run(const problem_t* p, uint64_t seed,
    ...
    /* C59-MP : Seeds indépendants par réplique — évite la race condition OpenMP sur seed */
    uint64_t seed_r[PT_MC_N_REPLICAS];
    for (int r = 0; r < R; ++r)
        seed_r[r] = seed ^ ((uint64_t)(r + 1) * 0x9e3779b97f4a7c15ULL);
    ...
    for (int sw = 0; sw < PT_MC_N_THERMALIZE; ++sw) {
        ...
        /* C59-MP : Répliques en parallèle — chaque thread sur un cœur distinct */
        #pragma omp parallel for reduction(+:mc_acc_th) schedule(static) num_threads(R)
        for (int r = 0; r < R; ++r) {
            double beta = beta_rep[r];
            double acc  = 0.0;
            for (int step = 0; step < N_STEP; ++step) {
                int idx = (int)(rand01(&seed_r[r]) * (double)sites);
                if (idx >= sites) idx = sites - 1;
                double d_old = d_rep[r][idx];
                double d_new = d_old + delta_mc * (2.0 * rand01(&seed_r[r]) - 1.0);
    ...
    for (int sw = 0; sw < N_SW; ++sw) {
        ...
        /* --- Sweeps Metropolis par réplique — C59-MP : parallèle sur les cœurs --- */
        #pragma omp parallel for reduction(+:mc_acc_sw) schedule(static) num_threads(R)
        for (int r = 0; r < R; ++r) {
            double beta = beta_rep[r];
            double acc  = 0.0;
            for (int step = 0; step < N_STEP; ++step) {
...
#undef FOPEN_DIAG
    ...
    /* C59-setvbuf : Buffers I/O dynamiques 70% RAM dispo — maximise throughput disque */
    {
        long avail = mem_available_kb() * 1024L;
        long ram_target = (long)((double)avail * 0.70);
        int n_csv = 15;
        long buf_per_file = (ram_target > 0 && n_csv > 0) ? ram_target / n_csv : 4L * 1024L * 1024L;
        if (buf_per_file < 1L * 1024L * 1024L)  buf_per_file = 1L * 1024L * 1024L;
        if (buf_per_file > 32L * 1024L * 1024L) buf_per_file = 32L * 1024L * 1024L;
        FILE* csv_files[] = { raw, tcsv, qcsv, prov, bcsv, bcsvm, mmeta, det, nstab, toy, tdrv, ucsv, ngcsv, dmcsv, lg };
        for (int fi = 0; fi < n_csv; fi++)
            if (csv_files[fi]) setvbuf(csv_files[fi], NULL, _IOFBF, (size_t)buf_per_file);
        fprintf(stderr, "[C59-setvbuf] buf_per_file=%ldMB n_files=%d RAM_avail=%ldMB RAM_target=%ldMB\n",
                buf_per_file / (1024L * 1024L), n_csv,
                avail / (1024L * 1024L), ram_target / (1024L * 1024L));
    }
