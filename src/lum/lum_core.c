...
void lum_group_destroy(lum_group_t* group) {
    if (group) {
        if (group->lums) {
            free(group->lums);
            group->lums = NULL;  // Protection double-free
        }
        free(group);
    }
}

// Fonction sécurisée pour destruction avec protection double-free
void lum_group_safe_destroy(lum_group_t** group_ptr) {
    if (group_ptr && *group_ptr) {
        lum_group_destroy(*group_ptr);
        *group_ptr = NULL;  // Protection double-free systématique
    }
}
