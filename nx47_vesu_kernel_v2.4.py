...
class NX47_VESU_Production:
    ...
    @staticmethod
    def _matching_wheels(package_name: str, wheel_roots: Sequence[Path]) -> List[Path]:
        matches: List[Path] = []
    ...
    def install_offline(self, package_name: str, required: bool = True) -> bool:
        ...
        # Avoid noisy pip errors for optional packages when no wheel exists at all.
        if not required and not self._matching_wheels(package_name, wheel_roots):
            self.log_event("OPTIONAL_DEPENDENCY_MISSING", {"package": package_name, "reason": "wheel_not_found"}, severity="WARN")
            return False
    ...
    def validate_source_indentation(source_path: Optional[str] = None) -> None:
        ...
        if source_path:
            ...
            candidates.append(Path.cwd() / "nx47_vesu_kernel_v2.4.py")
    ...
    def _read_tiff_volume(self, path: str) -> "np.ndarray":
        ...
        try:
            ...
            if "requires the 'imagecodecs' package" not in str(exc):
                raise
            # Reuse validated legacy fallback path: Pillow decoder for LZW TIFF.
            if Image is None or ImageSequence is None:
                raise FatalPipelineError(
                    f"TIFF_LZW_UNSUPPORTED: {path} requires imagecodecs and Pillow fallback is unavailable"
                ) from exc
            with Image.open(path) as img:
                frames = [np.array(frame, dtype=np.float32) for frame in ImageSequence.Iterator(img)]
            if not frames:
                raise FatalPipelineError(f"TIFF_DECODE_EMPTY: no frames decoded from {path}") from exc
